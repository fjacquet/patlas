 ...
 *
 * `extractStackedSection` CANNOT be used here: the TOC at the top of the
 * sheet also contains a single-cell "Pools" row, which the helper would
 * pick as the section title instead of the real one further down. This
 * custom scanner locates the actual header row by requiring ≥4 non-empty
 * cells so the TOC index entry (1 non-empty cell) is skipped.
 */
