...
describe('adaptProxmoxStorageContent', () => {
  ...
  it('parses a disk image row with branded MiB size and serial date', () => {
    ...
    expect(r?.contentType).toBe('images')
    ...
    expect(r?.creationDateSerial).toBe(46190.36)
  })
  ...
  it('handles ISO row with blank guestId and guestName', () => {
    ...
    expect(rows[0]?.contentType).toBe('iso')
  })
  ...
  it('handles null Creation Date as null', () => {
    const rows = adaptProxmoxStorageContent(
      sheet([
        {
          Node: 'promox',
    ...
    expect(rows).toHaveLength(1)
    expect(rows[0]?.creationDateSerial).toBeNull()
  })
})
