...
patlas is a 100% client-side React 19 + Vite 8 single-page application. It ingests one or more Proxmox VE reports (as `.zip` bundles or bare `.xlsx` files) entirely in the browser, normalizes them into typed snapshot rows, merges the selected snapshots into a single logical Proxmox estate, derives every aggregate (totals, allocation ratios, per-cluster/per-node/per-storage breakdowns, snapshot sprawl, storage content, cluster health, RRD headroom/growth, protection pack, governance posture, network topology) as pure functions, and renders dashboards, inventory trees, node views, capacity planning, and a full set of Proxmox-native views. No report byte ever leaves the browser — a runtime privacy guard throws synchronously on any non-same-origin network attempt. The architecture is a strict three-tier separation: a pure-function **engines** layer (no React/DOM/Zustand), an inputs-only **Zustand store**, and a UI layer bridged by exactly one `useMemo` site (`useEstateView`). Engineering principles are binding: KISS, DRY, functional programming — engines are pure, the store holds inputs only, and derived state is computed downstream, never cached. patlas is a fork of [vatlas](https://github.com/fjacquet/vatlas); the core spine is unchanged; the domain is remapped from VMware/RVTools to Proxmox.
...
graph TD
  ...
  AGG --> RRDNODES[engines/aggregation/rrdNodeStats.ts<br/>RRD CPU/mem/net headroom]
  AGG --> RRDSTOR[engines/aggregation/rrdStorageGrowth.ts<br/>RRD storage growth trends]
  AGG --> FSRISK[engines/aggregation/fsFillRisk.ts<br/>in-guest FS fill risk]
  AGG --> DISKHY[engines/aggregation/diskHygiene.ts<br/>disk hygiene]
  AGG --> BKCOV[engines/aggregation/backupCoverage.ts<br/>backup coverage]
  AGG --> GOVERN[engines/aggregation/governance.ts<br/>governance posture]
  AGG --> STBYRL[engines/aggregation/storageByRole.ts<br/>storage by role VM/backup/local]
  AGG --> NETW[engines/aggregation/network.ts<br/>network topology rollup]
  AGG --> UNITS[engines/units<br/>branded MiB/GHz/Cores]

  UEV -->|EstateView props| VIEWS[GlobalDashboard / InventoryView<br/>HostsView / PlanningView / EosView / TrendsView]
  UEV -->|EstateView.snapshotSprawl| SNP[components/snapshots/SnapshotSprawlView]
  ...
  UEV -->|EstateView.rrdHeadroom + rrdStorageGrowth| RRD[components/rrd/RrdHeadroomView<br/>StorageGrowthView]
  UEV -->|fsFillRisk + diskHygiene + backupCoverage| PROT[components/protection/ProtectionView]
  UEV -->|EstateView.governance| GOV[components/governance/GovernanceView]
  UEV -->|EstateView.network| NET[components/network/NetworkView]
  UEV -->|EstateView.storage| STOR[components/storage/StorageView]
  UEV -->|EstateView.sizing + monsters| RSZ[components/rightsizing/RightSizingView<br/>components/monstervm/MonsterVmView]
...
5. **Derive the estate view.** Components call `useEstateView(mode: AccountingMode): EstateView` (`src/hooks/useEstateView.ts`) — the project's single sanctioned `useMemo` site. The planned allocation ratios are read from the store inside the hook (via `selectPlannedRatios`), not passed as an argument. Inside the one memo it filters the store's `Snapshot` Map by `selectedSnapshotIds`, calls `mergeSnapshotsToEstate(selected)` to flatten and dedupe rows into one `MergedEstate`, then `buildEstateView(merged, mode, opts)` which composes per-cluster/per-node/per-storage aggregates plus all view slices (`snapshotSprawl`, `storageContent`, `clusterHealth`, `rrdHeadroom`, `rrdStorageGrowth`, `fsFillRisk`, `diskHygiene`, `backupCoverage`, `governance`, `network`, `sizing`, `monsters`). Returns the frozen `EMPTY_VIEW` when nothing is selected.
6. **Render.** `App.tsx` switches between `GlobalDashboard`, `InventoryView`, `HostsView`, `PlanningView`, `EosView`, `TrendsView`, `StorageView`, `NetworkView`, `RightSizingView`, `MonsterVmView`, `SnapshotSprawlView`, `StorageContentView`, `ClusterHealthView`, `RrdHeadroomView`, `StorageGrowthView`, `ProtectionView`, and `GovernanceView` based on `ViewToggle` state (nav ids: `dashboard`, `inventory`, `hosts`, `rrdheadroom`, `planning`, `eos`, `trends`, `storage`, `storagegrowth`, `network`, `rightsizing`, `monstervm`, `snapshots`, `storagecontent`, `clusterhealth`, `protection`, `governance`). Each view consumes the memoized `EstateView` as plain props and must not introduce its own `useMemo`. Charts route through the single `components/Chart.tsx` primitive, which injects the SVG renderer and the Midnight Executive theme.
...
| `rrdNodeStats` | `src/engines/aggregation/rrdNodeStats.ts` | Pure: aggregates Proxmox RRD node time-series rows into per-node CPU/memory/net headroom. Fixes the prior hardcoded-0 CPU — all utilisation now derives from cv4pve RRD sheets. |
| `rrdStorageGrowth` | `src/engines/aggregation/rrdStorageGrowth.ts` | Pure: aggregates Proxmox RRD storage time-series rows into fill-rate trends and projected full dates. |
| `fsFillRisk` | `src/engines/aggregation/fsFillRisk.ts` | Pure: identifies in-guest filesystems over the configured fill threshold. |
| `diskHygiene` | `src/engines/aggregation/diskHygiene.ts` | Pure: flags unused/orphaned/oversized Proxmox storage disks. |
| `backupCoverage` | `src/engines/aggregation/backupCoverage.ts` | Pure: measures guest backup coverage and operational health from cluster-task data. |
| `governance` | `src/engines/aggregation/governance.ts` | Pure: aggregates access posture (users/tokens/roles/realms) and issue counts into `GovernancePosture`. |
| `storageByRole` | `src/engines/aggregation/storageByRole.ts` | Pure: groups `StorageRow[]` by cv4pve role (VM data / backup / local) so per-role capacity figures are meaningful rather than an undifferentiated total. |
| `network` | `src/engines/aggregation/network.ts` | Pure: aggregates Proxmox network sheets (NICs/bonds/bridges/VLANs) into a `NetworkRollup`. |
...
| `EstateView` (and `AccountingMode`) | `src/types/estate.ts` | The typed contract every dashboard component and export consumes. Includes slices: `.sizing`, `.monsters`, `.snapshotSprawl`, `.storageContent`, `.clusterHealth`, `.rrdHeadroom`, `.rrdStorageGrowth`, `.fsFillRisk`, `.diskHygiene`, `.backupCoverage`, `.governance`, `.network`, `.storage`. `AccountingMode` is `'configured' \| 'active' \| 'storage-realistic'`. |
...
│   │   ├── rrdNodeStats.ts           RRD CPU/mem/net headroom per node
│   │   ├── rrdStorageGrowth.ts       RRD storage fill-rate trends
│   │   ├── fsFillRisk.ts             In-guest FS fill risk
│   │   ├── diskHygiene.ts            Disk hygiene flags
│   │   ├── backupCoverage.ts         Backup coverage + operational health
│   │   ├── governance.ts             Governance posture (users/tokens/roles)
│   │   ├── storageByRole.ts          Storage grouped by cv4pve role (VM data/backup/local)
│   │   ├── network.ts                Network topology rollup (NIC/bond/bridge/VLAN)
...
├── components/           UI. Subfolders per view: dashboard/, inventory/, hosts/, planning/, cluster/
│   ├── rrd/              RrdHeadroomView, StorageGrowthView (RRD-derived capacity/trends)
│   ├── snapshots/        SnapshotSprawlView
│   ├── storagecontent/   StorageContentView
│   ├── clusterhealth/    ClusterHealthView
│   ├── protection/       ProtectionView (fsFillRisk + diskHygiene + backupCoverage)
│   ├── governance/       GovernanceView
│   ├── storage/          StorageView (role-segmented capacity)
│   ├── network/          NetworkView
│   ├── rightsizing/      RightSizingView
│   ├── monstervm/        MonsterVmView
│   ├── eos/              EosView
│   ├── trends/           TrendsView
...
- **`components/` is split per view** so each top-level navigation target (dashboard, inventory, hosts, rrdheadroom, planning, eos, trends, storage, storagegrowth, network, rightsizing, monstervm, snapshots, storagecontent, clusterhealth, protection, governance) is self-contained, plus shared cross-view folders (`cluster/`).
- **The Proxmox-native view set** (`snapshots/`, `storagecontent/`, `clusterhealth/`, `rrd/`, `protection/`, `governance/`) is web-only. Each view consumes `EstateView` slices from `useEstateView` and is not wired into the HTML report or PPTX export engines.
