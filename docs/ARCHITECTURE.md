...
├── components/           UI. Subfolders per view: dashboard/, inventory/, hosts/, planning/, cluster/, dr/, stretched/, allocation/
...
- **`components/` is split per view** so each top-level navigation target (dashboard, inventory, hosts, planning) is self-contained, plus shared cross-view folders (`cluster/`, `dr/`, `stretched/`, `allocation/`).
