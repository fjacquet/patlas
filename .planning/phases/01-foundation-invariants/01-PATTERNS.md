...
2. **Multi-file wiring:** `multiple` attribute (line 123) stays. The `onFiles` callback now dispatches to `useSnapshotUpload` (Plan 5), which parses each file independently in the worker and adds N snapshots to the store.

3. **Locale key:** `t('upload:dropzone.accepted')` text in `upload.json` is changed to "Accepted format: .xlsx (RVTools export). Drop one file or several at once.".
